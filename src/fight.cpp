...
static void thunk(THING*, const char*, bool);
static void hit(const char*, const char*, bool);
static void bounce(THING*, const char*, bool);
static void miss(const char*, const char*, bool);
...
prname(const char* mname, bool upper)
{
    ...
    *tbuf = '\0';
    if (mname == nullptr)
    {
        ...
        std::strcpy(tbuf, mname);
    }
    if (upper)
    {
        *tbuf = std::toupper(*tbuf);
    }
