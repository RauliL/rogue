...
static void doadd(const char*, std::va_list);
 ...
 */
static void
doadd(const char* fmt, std::va_list args)
{
    ...
    if (std::strlen(buf) + newpos >= MAXMSG)
    {
        endmsg();
    }
    std::strcat(msgbuf, buf);
    newpos = static_cast<int>(std::strlen(msgbuf));
}
